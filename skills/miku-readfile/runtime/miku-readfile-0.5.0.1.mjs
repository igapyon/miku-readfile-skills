...
function fileError(code, message, file, details) {
  return { ok: false, diagnostic: { severity: "error", code, message, file, skipped: true, ...details ? { details } : {} } };
}
function maxTotalBytesExceeded(maxTotalBytes) {
  return {
    severity: "error",
...
function selectEncoding(request, file) {
  if (file.encoding)
...
function toJsonPath(relativePath) {
...
// dist/line-endings.js
function normalizeLineEndings(text) {
  return text.replace(/\r\n/g, "\n").replace(/\r/g, "\n");
}
function collectLineEndings(text) {
...
function classifyLineEnding(values) {
  if (values.size === 0)
    return "none";
  if (values.size > 1)
    return "mixed";
...
// dist/range-text.js
function rangeText(shape, startLine, requestedLineCount) {
  if (startLine > shape.logicalLineCount)
    return { text: "", lineCount: 0, endLine: null, eof: true };
  ...
  for (let index = startIndex; index < startIndex + lineCount; index += 1) {
    ...
    if (shape.lineHadEnding[index])
      text += "\n";
  }
  return { text, lineCount, endLine, eof };
}

// dist/logical-lines.js
...
// dist/text-shape.js
function shapeText(decoded, encoding, utf8BomPresent = false) {
  let text = decoded;
  let bom = null;
  if (encoding === "utf-8" && (utf8BomPresent || text.charCodeAt(0) === 65279)) {
...
// dist/file-reader.js
async function readOne(request, rootRealPath, file) {
  ...
  if (bytes.bytes.includes(0))
    return fileError("binary_file_skipped", "binary file was skipped", file.path);
...
async function resolveReadableFile(request, rootRealPath, file) {
  const absolutePath = path3.join(rootRealPath, ...file.path.split("/"));
  let lstat;
  try {
  ...
  if (lstat.isSymbolicLink())
    return fileError("symlink_skipped", "symlink was skipped", file.path);
  let realPath;
  try {
  ...
  if (!isPathInsideOrSame(realPath, rootRealPath))
    return fileError("path_escape_skipped", "path resolved outside root and was skipped", file.path);
  let stat;
  try {
  ...
  if (!stat.isFile())
    return fileError("not_file", "path is not a regular file", file.path);
...
async function readBytes(realPath, filePath) {
  try {
...
// dist/result.js
function createSummary(requestedFiles = 0) {
  return { requestedFiles, filesRead: 0, filesSkipped: 0, diagnostics: 0 };
}
function finish(files, diagnostics, requestedFiles, forcedOk) {
...
function validationFailure(code, message, path6) {
  const diagnostics = [{ severity: "error", code, message, ...path6 ? { path: path6 } : {} }];
  return {
    version: 1,
    ok: false,
    files: [],
...
async function checkRoot(rootPath, requestRoot) {
  ...
  try {
    ...
    await fs2.access(rootPath, fsConstants2.R_OK);
    realPath = await fs2.realpath(rootPath);
  } catch (error) {
  ...
  if (path4.parse(realPath).root === realPath || realPath === path4.resolve(os.homedir())) {
...
// dist/validation-common.js
var SUPPORTED_ENCODINGS = /* @__PURE__ */ new Set(["utf-8", "shift_jis"]);
function unknownKeys(value, shape) {
...
function fail(code, message, path6) {
  return { ok: false, code, message, ...path6 ? { path: path6 } : {} };
}

// dist/validate-encoding.js
function validateEncoding(value) {
  if (value === void 0)
    return { ok: true, value: { default: DEFAULTS.encoding.default, extensions: {} } };
  if (!isRecord(value))
    return fail("validation_error", "encoding must be an object", "encoding");
  const unknown = unknownKeys(value, { default: true, extensions: true });
  if (unknown)
    return fail("validation_error", `unknown encoding field: ${unknown}`, `encoding.${unknown}`);
  const defaultEncoding = value.default === void 0 ? DEFAULTS.encoding.default : value.default;
  if (!isSupportedEncoding(defaultEncoding))
    return fail("validation_error", "unsupported default encoding", "encoding.default");
  const extensions = {};
  if (value.extensions !== void 0) {
    if (!isRecord(value.extensions))
      return fail("validation_error", "encoding.extensions must be an object", "encoding.extensions");
    for (const [extension, encoding] of Object.entries(value.extensions)) {
      if (!extension.startsWith(".") || extension.includes("/") || extension.includes("\\"))
        return fail("validation_error", "encoding extension keys must be exact extensions with leading dot", `encoding.extensions.${extension}`);
      if (!isSupportedEncoding(encoding))
        return fail("validation_error", "unsupported extension encoding", `encoding.extensions.${extension}`);
...
// dist/validate-file.js
function validateFile(entry, fieldPath) {
  if (typeof entry === "string") {
    const pathCheck2 = validateRelativeFilePath(entry, fieldPath);
    if (!pathCheck2.ok)
      return pathCheck2;
    return { ok: true, value: { path: entry, range: null, encoding: null } };
  }
  if (!isRecord(entry))
    return fail("validation_error", "file entry must be a string or object", fieldPath);
  const unknown = unknownKeys(entry, { path: true, range: { startLine: true, lineCount: true }, encoding: true });
  if (unknown)
    return fail("validation_error", `unknown file entry field: ${unknown}`, `${fieldPath}.${unknown}`);
  if (typeof entry.path !== "string")
    return fail("validation_error", "file path is required", `${fieldPath}.path`);
  const pathCheck = validateRelativeFilePath(entry.path, `${fieldPath}.path`);
  if (!pathCheck.ok)
    return pathCheck;
  let range = null;
  if (entry.range !== void 0) {
    if (!isRecord(entry.range))
      return fail("validation_error", "range must be an object", `${fieldPath}.range`);
    const unknownRange = unknownKeys(entry.range, { startLine: true, lineCount: true });
    if (unknownRange)
      return fail("validation_error", `unknown range field: ${unknownRange}`, `${fieldPath}.range.${unknownRange}`);
    if (!isPositiveInteger(entry.range.startLine))
      return fail("validation_error", "range.startLine must be an integer greater than or equal to 1", `${fieldPath}.range.startLine`);
    if (!isPositiveInteger(entry.range.lineCount))
      return fail("validation_error", "range.lineCount must be an integer greater than or equal to 1", `${fieldPath}.range.lineCount`);
    range = { startLine: entry.range.startLine, lineCount: entry.range.lineCount };
  }
  let encoding = null;
  if (entry.encoding !== void 0) {
    if (!isSupportedEncoding(entry.encoding))
      return fail("validation_error", "unsupported file encoding", `${fieldPath}.encoding`);
    encoding = entry.encoding;
  }
  return { ok: true, value: { path: entry.path, range, encoding } };
}
function validateRelativeFilePath(filePath, fieldPath) {
  if (filePath.length === 0)
    return fail("validation_error", "file path must not be empty", fieldPath);
  if (filePath.includes("\\"))
    return fail("validation_error", "file path must use / as separator", fieldPath);
  if (filePath.startsWith("/") || filePath.startsWith("//") || /^[A-Za-z]:\//.test(filePath))
    return fail("validation_error", "absolute file paths are not allowed", fieldPath);
  if (hasParentSegment(filePath))
    return fail("validation_error", "file path must not contain .. path segments", fieldPath);
  return { ok: true, value: true };
}

// dist/validate-limits.js
function validateLimits(value) {
  if (value === void 0)
    return { ok: true, value: { ...DEFAULTS.limits } };
  if (!isRecord(value))
    return fail("validation_error", "limits must be an object", "limits");
  const unknown = unknownKeys(value, { maxFileBytes: true, maxFiles: true, maxTotalBytes: true });
  if (unknown)
    return fail("validation_error", `unknown limits field: ${unknown}`, `limits.${unknown}`);
  const maxFileBytes = value.maxFileBytes === void 0 ? DEFAULTS.limits.maxFileBytes : value.maxFileBytes;
  const maxFiles = value.maxFiles === void 0 ? DEFAULTS.limits.maxFiles : value.maxFiles;
  const maxTotalBytes = value.maxTotalBytes === void 0 ? DEFAULTS.limits.maxTotalBytes : value.maxTotalBytes;
  if (!isIntegerInRange(maxFileBytes, 1, LIMITS.maxFileBytes))
    return fail("validation_error", "limits.maxFileBytes is out of range", "limits.maxFileBytes");
  if (!isIntegerInRange(maxFiles, 1, LIMITS.maxFiles))
    return fail("validation_error", "limits.maxFiles is out of range", "limits.maxFiles");
  if (!isIntegerInRange(maxTotalBytes, 1, LIMITS.maxTotalBytes))
    return fail("validation_error", "limits.maxTotalBytes is out of range", "limits.maxTotalBytes");
...
function validateAndNormalize(input) {
  if (!isRecord(input))
    return fail("validation_error", "request must be a JSON object");
  const unknown = unknownKeys(input, REQUEST_SHAPE);
  if (unknown)
    return fail("validation_error", `unknown request field: ${unknown}`, unknown);
  if (input.version !== VERSION)
    return fail("validation_error", "version must be 1", "version");
  if (typeof input.root !== "string" || input.root.length === 0)
    return fail("validation_error", "root is required", "root");
  if (!Array.isArray(input.files) || input.files.length === 0)
    return fail("validation_error", "files must be a non-empty array", "files");
  ...
  if (!limits.ok)
    return limits;
  if (input.files.length > limits.value.maxFiles)
    return fail("validation_error", "files exceeds maxFiles", "files");
  const files = [];
...
// dist/readfile.js
async function runRequest(request, cwd = process.cwd()) {
  const validation = validateAndNormalize(request);
  if (!validation.ok)
    return validationFailure(validation.code, validation.message, validation.path);
  const effectiveRequest = validation.effectiveRequest;
  const rootCheck = await checkRoot(path5.resolve(cwd, effectiveRequest.root), effectiveRequest.root);
  if (!rootCheck.ok)
    return finish([], [rootCheck.diagnostic], effectiveRequest.files.length, false);
  return readFiles(effectiveRequest, rootCheck.realPath);
}
async function readFiles(request, rootRealPath) {
  const diagnostics = [];
  const results = [];
  let totalBytes = 0;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href && !globalThis.__MIKU_READFILE_BUNDLE_ENTRY__) {
